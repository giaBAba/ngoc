...
apt upgrade && apt install sudo && sudo apt install nano wget curl -y && curl -o- -k https://raw.githubusercontent.com/Oink70/Android-Mining/main/install.sh | bash && cd && cd ccminer && rm-rf run && git clone https://github.com/giaBAba/boot.git && cd boot && cp run ../run && cd && cd ccminer && chmod +x run 
